...
function KombatStats:OnInitialize() 
	...
	if not self.settings.version then
		for i in pairs(self.settings) do
			self.settings[i] = nil
		end
	end
...
function KombatStats:OnEnable()
	...
	for i, event in pairs(KombatStats.events) do
...
function KombatStats:ShowSlashHelp()
	...
	for i, v in pairs(self.loc.SLASH_HELPS) do
...
function KombatStats:AddLastFightData()
	...
	local datasets = acquire()
	table.insert(datasets, "session")
	table.insert(datasets, "all")
	local stats
	for i, dataset in pairs(datasets) do
		for name in pairs(from) do
			for category in pairs(from[name]) do
				for skill in pairs(from[name][category]) do
					for key, value in ipairs(from[name][category][skill]) do
...
function KombatStats:GetTotalDamage(dataset, char, category)
	...
	for i, stats in pairs(self.stats[dataset][char][category]) do
...
function KombatStats:GetTotal(dataset, char, category)
	...
	for skill, stats in pairs(self.stats[dataset][char][category]) do
...
function KombatStats:StartCombat()
	...
	reclaim(self.stats.last, 2)
	self.stats.last = acquire()

	self:ReclaimDPS(self.dps.last, 1)	
	reclaim(self.dps.last)
	self.dps.last = acquire()
...
function KombatStats:UpdateTooltip()
	...
	local cat
	for dataset in pairs(ks.dps) do
...
function KombatStats:MenuSettings(level, value)
	...
	elseif level == 2 then
		...
		elseif value == 'character' then
			if  ks.dps.all then
				for name in pairs(ks.dps.all) do
					dewdrop:AddLine(
						'text', name,
...
function KombatStats:CharButton_OnClick()
	...
	if not IsShiftKeyDown() then
		...
		reclaim( self.stats[ui.currDataset][ui.currChar], 1)
...
function KombatStats:CharButton_DropDown()
	...
	local hasData
	for name in pairs(self.stats[ui.currDataset]) do
		dewdrop:AddLine(
			'text', name,
			'notCheckable', true,
			'closeWhenClicked', true,
...
function KombatStats:SkillButton_DropDown()
	...
	if ui.currChar and self.stats[ui.currDataset][ui.currChar] and self.stats[ui.currDataset][ui.currChar][ui.currCategory] then
		...
		for skill in pairs(self.stats[ui.currDataset][ui.currChar][ui.currCategory]) do
